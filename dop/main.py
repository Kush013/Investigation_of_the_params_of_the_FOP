...
"""
# tim[check, calibr] tch- время калибровки, tca - время проверки(check)
# circles[0-4] - 0-цт, 1-i, 2-ii, 3-iii, 4-iv
# yark_cal - калибровка яркости
file_img, calibr, yark_cal, image, blur, tch, tca, circles, thresh = None, [], None, None, None, None, None, [], None
TIME_LIMIT = 100
nastr = []
...
def yark_func(d, image_gray):
    ...
    mask = cv2.circle(mask, (int(a / 2), int(a / 2)), int(a / 2), (255, 255, 255), -1)
...
# Главная функция для ПРОВЕРКИ яркости, остальные вызываются отсюда. перевод в проценты
def yark_out(circ):
    y = yark(circ)
...
def menu(chbx):
    ...
    if chbx[1]:
        d = cv2.HoughCircles(thresh, cv2.HOUGH_GRADIENT, 1.5, 700, param1=nastr[0], param2=nastr[1],
                             minRadius=nastr[2], maxRadius=5000)
        if d is None:
...
class mywindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.s_w = None
        self.setWindowTitle("Поворот/кривизна/смещение/яркость")
        self.setWindowIcon(QIcon("dop/logo.ico"))
    ...
    def open(self):
        ...
        Image.open(file_img[0]).resize((854, 641)).save("dop/out_res.jpeg")
        pixmap = QPixmap("dop/out_res.jpeg")
        fl = file_img[0].split("/")
    ...
    def nastr_krugi(self):
        # print(nastr)
        circ = None
        cv2.imwrite("thresh3.jpeg", thresh)
    ...
    def nastr_ok(self):
        with open("settings.txt", 'r+') as f:
            ...
            f.write("{}\n{}\n{}\n{}".format(nastr[0], nastr[1], nastr[2], nastr[3]))
